...
class GPIOSimulator:
    ...
    def show_leds_states(self):
        ...
        for i in range(N_LEDS):
            comma = '' if i == 0 else ','
            if state_strs[self.__led_states[i]] == "OFF":
                msg += colored("%s  %d: %s" % (comma, i,
                                               state_strs[self.__led_states[i]]),
                               self.colours[6], "on_grey")
            else:
                msg += colored("%s  %d: %s" % (comma, i,
                                               state_strs[self.__led_states[i]]),
                               self.colours[i], "on_grey")
        msg += ']'
        print(msg)
        self.__led_states = [self.OFF] * N_LEDS
